...
function App() {
	...
	const addTodo = (todo: Partial<Todo>) => {
		setTodos((prevTodos) => [{ ...todo, id: nanoid() }, ...prevTodos]);
	};

	const updateTodo = (id: string, todo: Partial<Todo>) => {
		setTodos((prevTodos) =>
			prevTodos.map((prevTodo) =>
				prevTodo.id === id ? { ...todo, id: prevTodo.id } : prevTodo
			)
		);
	};
	...
	return (
		<>
			...
			<TodoProvider
				value={{ todos, addTodo, updateTodo, toggleTodo, removeTodo }}
			>
				<div className="font-mono hero content-between min-h-screen bg-base-200">
